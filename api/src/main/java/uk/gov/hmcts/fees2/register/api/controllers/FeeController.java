...
public class FeeController {
    ...
    public List<Fee2Dto> search(@RequestParam(required = false) String service,
        ...
        return feeService
            .search(new LookupFeeDto(service, jurisdiction1, jurisdiction2, channel, event, application, amount, unspecifiedClaimAmounts, feeVersionStatus, author))
            .stream()
            .filter(f -> {
    ...
    public ResponseEntity<FeeLookupResponseDto> lookup(@RequestParam String service,
        ...
        final FeeLookupResponseDto responseDto = feeService.lookup(new LookupFeeDto(service, jurisdiction1, jurisdiction2, channel, event, application, amountOrVolume, false, FeeVersionStatus.approved, null));
    ...
    public FeeLookupResponseDto lookupUnspecified(@RequestParam String service,
                                                  ...
                                                  @RequestParam String application,
                                                  HttpServletResponse response) {
        return feeService.lookup(new LookupFeeDto(service, jurisdiction1, jurisdiction2, channel, event, application, null, true, FeeVersionStatus.approved, null));
    }
