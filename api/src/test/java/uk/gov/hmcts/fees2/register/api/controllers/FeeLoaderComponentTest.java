...
public class FeeLoaderComponentTest extends BaseTest {
    ...
    public void testFeeLoaderForCmcOnlineFees() throws Exception {
        restActions
            ...
            .andExpect(status().isOk())
            .andExpect(body().asListOf(Fee2Dto.class, fee2Dtos -> {
                Fee2Dto fee = fee2Dtos.stream().filter(f -> f.getCode().equals("FEE0217")).findAny().get();
                assertThat(fee.getCode()).isEqualTo("FEE0217");
                assertThat(fee.getChannelTypeDto().getName()).isEqualTo("online");
                assertThat(fee.getServiceTypeDto().getName()).isEqualTo("civil money claims");
                assertThat(fee.getJurisdiction1Dto().getName()).isEqualTo("civil");
                assertThat(fee.getJurisdiction2Dto().getName()).isEqualTo("county court");
                assertThat(fee.getMinRange()).isEqualTo(new BigDecimal("5000.01"));
                assertThat(fee.getMaxRange()).isEqualTo(new BigDecimal("10000.00"));
            }));
    }
